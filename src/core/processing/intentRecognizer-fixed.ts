...
export class IntentRecognizer {
  ...
  async recognizeIntent(message: Message, context: MessageContext): Promise<MessageIntent> {
    try {
      ...
      const best = intents.reduce((best, current) => 
      ...
      // Extract entities
      const entities = this.extractEntities(content, best.type);

      const action = this.extractAction(content, best.type);
      const target = this.extractTarget(content, best.type);

      const result: MessageIntent = {
        type: best.type,
      ...
      this.logger.error('Error recognizing intent:', error as Error);
      return this.createIntent(IntentType.UNKNOWN, 0.0, []);
    }
  }
  ...
  private extractCommandEntities(content: string): IntentEntity[] {
    ...
    const userRegex = this.entityExtractors.get('user');
    if (userRegex) {
      const userMatches = Array.from(content.matchAll(userRegex));
  ...
  private extractGeneralEntities(content: string): IntentEntity[] {
    ...
    // Extract URLs
    const urlRegex = this.entityExtractors.get('url');
    if (urlRegex) {
